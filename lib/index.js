    ...
    ctx.effect(() => sys.variable('image_tiler_selection', () => {
      for (const task of tasks.values()) {
        ...
        if (Array.isArray(selected) && selected.length > 0) {
          return `用户已在工作台勾选 ${selected.length} 片切片（${selected.join(',')}）。当用户要求查看/分析该图片时：先调用 read_tiles（不要传参数）读取用户勾选的切片，基于切片内容回答；不要基于被缩小的原图回答。`
        }
      }
      return ''
