...
import { DEFAULTS, tileImage, tileLayout, ensureInside, selectTiles } from './tiler.js'
    ...
    handler(req, res) {
      ...
      void relay(req, res, async () => {
        ...
        if (route === 'slice') {
          const body = await readJsonBody(req)
          const cfg = resolveCfg()
          const outputDirAbs = resolve(s.cwd, body.output_dir ?? cfg.outputDir)
          ensureInside(s.cwd, outputDirAbs)
          const result = await tileImage(s.sourceAbs, {
            outputDirAbs,
            workspaceRoot: s.cwd,
            tileSize: body.tileSize ?? cfg.tileSize,
            overlap: body.overlap ?? cfg.overlap,
            format: body.format ?? cfg.format,
            label: body.label ?? cfg.label,
            overviewSize: body.overview_size ?? cfg.overviewSize,
            maxTiles: body.max_tiles ?? cfg.maxTiles,
          })
          task.state = {
            ...s,
            outputDirAbs,
