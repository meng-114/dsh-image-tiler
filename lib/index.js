...
import { DEFAULTS, tileImage, tileGrid, tileLayout, ensureInside, selectTiles, sanitizeName } from './tiler.js'
...
export const SETTINGS_SCHEMA = z.object({
  ...
  overlap: z.number().min(0).max(2048).default(0)
    .description('相邻切片之间的重叠像素数，默认为 0（无重叠）'),
...
function buildDescription(autoTile) {
  ...
  return base + ' When the user provides a large image (attachment or workspace path), automatically tile it first. After the user confirms tiles in the visual workbench (they may adjust the grid or pick specific tiles), call read_tiles with NO arguments so you read exactly the tiles they selected — never read the downscaled original when a selection exists.'
}
 ...
 * @param taskOf - per-session task resolver keyed by session id.
 * @returns the async execute function.
 */
function buildExecute(resolveCfg, taskOf) {
 ...
 * preview / slice / select / img / state / import. All paths stay inside the
 * session workspace, and every binary read is restricted to image extensions.
 * Tasks are keyed by session id so switching workspaces never bleed.
 * @param ctx - host plugin context.
 * @param taskOf - per-session task resolver.
 * @param resolveCfg - reads current settings (or DEFAULTS).
 */
function setupEndpoints(ctx, taskOf, resolveCfg) {
  ...
  const runSlice = async (cwd, sourceAbs, opts, task) => {
    const cfg = resolveCfg()
    ...
    return stateView(task)
  }

  // The webServer provider can mount after this plugin's apply (activation
  // order is not guaranteed and ctx.get is strict about the provider being
  // active), so register with a short retry loop like the settings wiring.
  ...
  const define = (s) => {
    s.register({
      ...
      void relay(req, res, async () => {
        const s = task.state
        if (route === 'state') {
          sendJson(res, 200, await stateView(task))
          return
        }
        ...
        if (route === 'preview') {
          ...
          const cfg = resolveCfg()
          const gridMode = Number.isSafeInteger(body.cols) && body.cols > 0 && Number.isSafeInteger(body.rows) && body.rows > 0
          const layout = gridMode
            ? tileGrid(s.sourceWidth, s.sourceHeight, body.cols, body.rows)
            : tileLayout(s.sourceWidth, s.sourceHeight, body.tileSize ?? cfg.tileSize, body.overlap ?? cfg.overlap)
          sendJson(res, 200, {
            layoutMode: gridMode ? 'grid' : 'size',
            cols: layout.cols,
            rows: layout.rows,
            count: layout.tiles.length,
            tiles: layout.tiles.map((tile) => ({ id: `r${tile.row + 1}c${tile.col + 1}`, row: tile.row + 1, col: tile.col + 1, x: tile.x, y: tile.y, width: tile.width, height: tile.height })),
          })
          return
        }
        if (route === 'slice') {
          const body = await readJsonBody(req)
          sendJson(res, 200, await runSlice(s.cwd, s.sourceAbs, body, task))
          return
        }
        if (route === 'import') {
          const body = await readJsonBody(req)
          ...
          sendJson(res, 200, await runSlice(cwd, targetAbs, body, task))
          return
        }
        if (route === 'select') {
          const body = await readJsonBody(req)
  ...
  const tryRegister = () => {
    ...
    try {
      ...
      console.error('image-tiler: workbench endpoint register failed', error && error.message)
    }
  }
  tryRegister()
  if (server === undefined) {
    ctx.effect(() => {
      let stopped = false
      const tick = () => {
        if (stopped || server !== undefined) return
        tryRegister()
        if (server === undefined) ctx.timeout(tick, 500)
      }
      tick()
      return () => { stopped = true }
    }, 'image-tiler: workbench endpoint activation')
  }
}
...
export function apply(ctx) {
  let scope
  /** Workbench tasks keyed by session id — switching workspaces never bleeds. */
  const tasks = new Map()
  const taskOf = (sessionId) => {
    const key = sessionId || 'default'
    if (!tasks.has(key)) tasks.set(key, { state: null })
    return tasks.get(key)
  }
  const resolveCfg = () => (scope !== undefined ? scope.get() : DEFAULTS)
  setupEndpoints(ctx, taskOf, resolveCfg)

  // Make the user's workbench selection visible to the model every turn:
  // a prompt variable referenced by a tiny section renders each assembly.
  const sys = ctx.get('systemPrompt')
  if (sys !== undefined) {
    ctx.effect(() => sys.variable('image_tiler_selection', () => {
      for (const task of tasks.values()) {
        if (task.state === null) continue
        const selected = task.state.selected
        if (Array.isArray(selected) && selected.length > 0) {
          return `用户已在工作台勾选 ${selected.length} 片切片（${selected.join(',')}）。当用户要求查看/分析该图片时：先调用 read_tiles（不要传参数）拿到缩略图与勾选清单，用 read_image 先看 overview 缩略图理解整体布局，再逐张读取勾选的切片；不要基于被缩小的原图回答。`
        }
      }
      return ''
    }), 'image-tiler: selection prompt variable')
    ctx.effect(() => sys.section({
      name: 'image-tiler-selection',
      order: 150,
      text: '{{image_tiler_selection}}',
    }), 'image-tiler: selection prompt section')
  }
  ...
  const registerTool = () => {
    ...
    const dispose = ctx.tools.register(defineTool({
      ...
      parameters: {
        ...
        overlap: {
          type: 'integer',
          description: 'Overlap between neighboring tiles in pixels (default 0).',
        },
        output_dir: {
          type: 'string',
        ...
        cols: {
          type: 'integer',
          description: 'Explicit column count (grid mode). When set with rows, the image is split into an even cols x rows grid instead of using tile_size.',
        },
        rows: {
          type: 'integer',
          description: 'Explicit row count (grid mode). When set with cols, the image is split into an even grid instead of using tile_size.',
        },
      },
      output: {
        schema: {
      ...
      execute: buildExecute(resolveCfg, taskOf),
      presentCall(args) {
        return {
          card: 'generic',
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const task = taskOf(exec.agent?.session?.id)
