...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** One tile thumbnail with checkbox. */
    function TileThumb(props) {
      ...
      return React.createElement('div', {
        ...
        React.createElement('img', { src: API + '/img?p=' + encodeURIComponent(tile.path), style: WB_STYLES.tileImg, alt: tile.id, loading: 'lazy' }),
    ...
    function TileWorkbench(props) {
      ...
      var loadState = function () {
        api('GET', '/state').then(state[1]).catch(function () { /* no task yet; card shows hint */ })
      }
      ...
      var refreshPreview = function (tileSize, overlap) {
        if (snap === null) return
        busy[1]('preview')
        api('POST', '/preview', { tileSize, overlap })
      ...
      var confirmSelection = function () {
        ...
        busy[1]('select')
        error[1]('')
        api('POST', '/select', { tiles: selectedIds })
          .then(function () {
            busy[1]('')
            error[1]('')
      ...
      var header = React.createElement('div', { style: WB_STYLES.row },
        ...
        React.createElement('input', {
          type: 'number', min: 64, max: 2048, step: 8, value: p.tileSize,
          onChange: function (e) {
            var v = Number(e.target.value)
            if (Number.isFinite(v)) setTileSize(Math.max(64, Math.min(2048, Math.round(v))))
          },
          style: Object.assign({}, WB_STYLES.input, { maxWidth: '84px' }),
          'aria-label': '切片边长（像素，可键盘输入）',
        }),
        ...
        React.createElement('input', {
          type: 'number', min: 0, max: 512, step: 8, value: p.overlap,
          onChange: function (e) {
            var v = Number(e.target.value)
            if (Number.isFinite(v)) setOverlap(Math.max(0, Math.min(512, Math.round(v))))
          },
          style: Object.assign({}, WB_STYLES.input, { maxWidth: '84px' }),
          'aria-label': '重叠像素（可键盘输入）',
        }),
        React.createElement('span', { style: WB_STYLES.num }, 'px'),
      ...
      if (snap !== null && gridRect !== null) {
        ...
        canvas = React.createElement('div', { style: WB_STYLES.canvas },
          React.createElement('img', { src: API + '/img?p=' + encodeURIComponent(snap.source.path), style: WB_STYLES.img, alt: 'source' }),
          React.createElement('svg', { viewBox: '0 0 ' + svgw + ' ' + svgh, style: WB_STYLES.overlay, 'aria-hidden': true },
            previewTiles.map(function (tile) {
              return React.createElement('rect', {
                ...
                stroke: 'rgba(77,141,255,0.85)',
                strokeWidth: Math.max(1, svgw / 600),
              })
            }),
            sepChildren,
          ),
        ...
        canvas = React.createElement('div', { style: WB_STYLES.canvas },
          React.createElement('img', { src: API + '/img?p=' + encodeURIComponent(snap.overview), style: WB_STYLES.img, alt: 'overview' }),
        )
      } else {
        canvas = React.createElement('div', { style: WB_STYLES.status }, '尚未切片——让模型调用 tile_image 后，这里会出现可视化切片工作台。')
      }

      // Tile picker grid (after slicing).
      var picker = null
    ...
    function DockWorkbench(props) {
      ...
      return React.createElement('div', { style: { width: '100%' } },
        ...
        !otherWs && open[0] ? React.createElement(TileWorkbench, { afterLoad: load }) : null,
      )
    }
    ...
    function apply(ctx) {
      ...
      ctx.effect(function () {
        var onDrop = function (event) {
          ...
          if (imgs.length === 0 || latestCwd === null) return
          var file = imgs[0]
          ...
          reader.onload = function () {
            ...
            if (!data) return
            api('POST', '/import', { filename: file.name || 'dropped.png', data, cwd: latestCwd })
