  ...
  factory: (require) => {
    ...
    function TileThumb(props) {
      ...
      return React.createElement('div', {
        ...
        React.createElement('img', { src: imgUrl(tile.path, props.session), style: WB_STYLES.tileImg, alt: tile.id, loading: 'lazy', draggable: false }),
    ...
    function TileWorkbench(props) {
      ...
      var params = React.useState({ tileSize: 800, overlap: 0, format: 'png', cols: 2, rows: 1 })
      var mode = React.useState('size')
      ...
      var refreshPreview = function (body) {
        if (snap === null) return
        busy[1]('preview')
        api('POST', '/preview', body, sessionId)
      ...
      var setTileSize = function (value) {
        ...
        params[1](next)
        mode[1]('size')
        refreshPreview({ tileSize: next.tileSize, overlap: next.overlap })
      }
      ...
      var setCols = function (value) {
        var next = Object.assign({}, p, { cols: Math.max(1, Math.min(24, Math.round(Number(value) || 1))) })
        params[1](next)
        mode[1]('grid')
        refreshPreview({ cols: next.cols, rows: next.rows })
      }
      var setRows = function (value) {
        var next = Object.assign({}, p, { rows: Math.max(1, Math.min(24, Math.round(Number(value) || 1))) })
        params[1](next)
        mode[1]('grid')
        refreshPreview({ cols: next.cols, rows: next.rows })
      }
      ...
      var doSlice = function () {
        busy[1]('slice')
        error[1]('')
        var body = mode[0] === 'grid'
          ? { cols: p.cols, rows: p.rows, format: p.format }
          : p
        api('POST', '/slice', body, sessionId)
          .then(function (data) {
            state[1](data)
            preview[1](null)
            selected[1]({})
      ...
      // Param rows: fixed-width controls, nowrap labels — no flex stretching.
      var PARAM_ROW = { display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }
      var P_LABEL = { fontSize: '12px', color: 'var(--dsw-alias-label-secondary, #999)', whiteSpace: 'nowrap', flex: 'none' }
      var P_RANGE = { flex: 'none', width: '150px' }
      var P_NUM = { width: '84px', padding: '4px 8px', borderRadius: '6px', border: '1px solid var(--dsw-alias-border-l2, rgba(128,128,128,.4))', background: 'transparent', color: 'inherit', fontSize: '13px', flex: 'none' }
      var P_NUMTEXT = { fontSize: '12px', flex: 'none', whiteSpace: 'nowrap' }
      var numCompact = function (key, value, min, max, setter, label) {
        return React.createElement('input', {
          type: 'number', min: min, max: max, step: 8,
          value: draft[key] !== undefined ? draft[key] : value,
          onChange: function (e) { numDraft[1](Object.assign({}, numDraft[0], { [key]: e.target.value })) },
      ...
      var paramsRow1 = React.createElement('div', { style: PARAM_ROW },
        React.createElement('span', { style: P_LABEL }, '切片边长'),
        React.createElement('input', { type: 'range', min: 64, max: 2048, step: 64, value: p.tileSize, onChange: function (e) { setTileSize(e.target.value) }, style: P_RANGE }),
        numCompact('tileSize', p.tileSize, 64, 2048, setTileSize, '切片边长（像素，可键盘输入）'),
        React.createElement('span', { style: P_NUMTEXT }, 'px'),
        React.createElement('span', { style: P_LABEL }, '重叠'),
        React.createElement('input', { type: 'range', min: 0, max: 512, step: 16, value: p.overlap, onChange: function (e) { setOverlap(e.target.value) }, style: P_RANGE }),
        numCompact('overlap', p.overlap, 0, 512, setOverlap, '重叠像素（可键盘输入）'),
        React.createElement('span', { style: P_NUMTEXT }, 'px'),
        React.createElement('span', { style: P_LABEL }, '列×行'),
        numCompact('cols', p.cols, 1, 24, setCols, '列数（等分网格）'),
        React.createElement('span', { style: P_NUMTEXT }, '×'),
        numCompact('rows', p.rows, 1, 24, setRows, '行数（等分网格）'),
      )
      var paramsRow2 = React.createElement('div', { style: PARAM_ROW },
        ...
        previewTiles ? React.createElement('span', { style: P_NUMTEXT },
          (preview[0].layoutMode === 'grid' ? '等分 ' : '') + preview[0].cols + ' 列 × ' + preview[0].rows + ' 行 = ' + preview[0].count + ' 片' + (preview[0].layoutMode === 'size' ? '（重叠 ' + p.overlap + 'px）' : '')) : null,
      )
      var header = React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '6px' } },
        paramsRow1,
        paramsRow2,
      )
      ...
      if (snap !== null && gridRect !== null) {
        ...
        canvas = React.createElement('div', { style: WB_STYLES.canvas },
          React.createElement('img', { src: imgUrl(snap.source.path, sessionId), style: WB_STYLES.img, alt: 'source', draggable: false }),
        ...
        canvas = React.createElement('div', { style: WB_STYLES.canvas },
          React.createElement('img', { src: imgUrl(snap.overview, sessionId), style: WB_STYLES.img, alt: 'overview', draggable: false }),
        )
      } else {
      ...
      var zoomOverlay = zoom[0] ? ReactDOM.createPortal(React.createElement('div', {
        style: { position: 'fixed', inset: '0', background: 'rgba(0,0,0,0.78)', zIndex: 100001, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px', cursor: 'zoom-out' },
        onClick: function () { zoom[1](null) },
      },
        React.createElement('img', { src: imgUrl(zoom[0].path, sessionId), style: { maxWidth: '92vw', maxHeight: '84vh', objectFit: 'contain', borderRadius: '8px' }, alt: zoom[0].id, draggable: false }),
    ...
    function DockWorkbench(props) {
      ...
      if (!otherWs && open[0]) {
        workbenchModal = ReactDOM.createPortal(React.createElement('div', {
      ...
      return React.createElement('div', { style: { width: 'calc(100% - 32px)', maxWidth: 'var(--dsh-composer-card-max-width, 960px)', margin: '0 auto' } },
