 ...
 * @module @mengli114/dsh-image-tiler/tiler
 */
...
/**
 * Compute an even grid of N x M tiles (no overlap; edge tiles absorb the
 * remainder). Used when the user picks columns/rows directly.
 * @param width - source width in pixels.
 * @param height - source height in pixels.
 * @param cols - column count.
 * @param rows - row count.
 * @returns the same grid shape as {@link tileLayout} (zero-based row/col).
 */
export function tileGrid(width, height, cols, rows) {
  const c = Math.max(1, Math.floor(cols))
  const r = Math.max(1, Math.floor(rows))
  const stepX = Math.floor(width / c)
  const stepY = Math.floor(height / r)
  const tiles = []
  for (let row = 0; row < r; row += 1) {
    const y = row * stepY
    const th = row === r - 1 ? height - y : stepY
    for (let col = 0; col < c; col += 1) {
      const x = col * stepX
      const tw = col === c - 1 ? width - x : stepX
      tiles.push({ row, col, x, y, width: tw, height: th })
    }
  }
...
/**
 * Named regions of a source image, as pixel windows. Used by read_tiles to
 * pick only the tiles a model actually needs.
 * @param region - one of 'center' (default), 'left', 'right', 'top', 'bottom', 'full'.
 * @param width - source width in pixels.
 * @param height - source height in pixels.
 * @returns the window rectangle in source pixel coordinates.
 */
export function regionWindow(region, width, height) {
  const halfW = Math.round(width * 0.5)
  const halfH = Math.round(height * 0.5)
  switch (region) {
    case 'full': return { x: 0, y: 0, w: width, h: height }
    case 'left': return { x: 0, y: 0, w: halfW, h: height }
    case 'right': return { x: halfW, y: 0, w: width - halfW, h: height }
    case 'top': return { x: 0, y: 0, w: width, h: halfH }
    case 'bottom': return { x: 0, y: halfH, w: width, h: height - halfH }
    case 'center':
    default: return { x: Math.round(width * 0.25), y: Math.round(height * 0.25), w: width - 2 * Math.round(width * 0.25), h: height - 2 * Math.round(height * 0.25) }
  }
}

/**
 * Select the tiles of a tiled layout that intersect a region window.
 * @param manifest - parsed manifest (needs `source.width/height` and `tiles[]`
 * with row/col/x/y/width/height).
 * @param region - named region ('center' default, or 'full').
 * @returns the selected tiles in manifest order.
 */
export function selectTiles(manifest, region = 'center') {
  const win = regionWindow(region, manifest.source.width, manifest.source.height)
  const tiles = manifest.tiles.filter((tile) => tile.x < win.x + win.w && tile.x + tile.width > win.x && tile.y < win.y + win.h && tile.y + tile.height > win.y)
  return { region, window: win, tiles }
}


/**
 * Slice one image into labeled tiles, an overview, and a manifest.
 * Images with a transparent background are auto-trimmed before slicing
 * (cropEdges, on by default) so invisible padding never becomes tiles.
...
export async function tileImage(inputAbs, options = {}) {
  ...
  // Trim fully-transparent edges so they never become tiles (PNG/WebP alpha).
  let imgSource = inputReal
  if (options.cropEdges !== false && meta.hasAlpha && meta.channels === 4) {
    const trimmed = await sharp(inputReal, { failOn: 'none' })
      .trim({ background: { r: 0, g: 0, b: 0, alpha: 0 } })
      .toBuffer({ resolveWithObject: true })
    if (trimmed.info.width < width || trimmed.info.height < height) {
      imgSource = trimmed.data
      width = trimmed.info.width
      height = trimmed.info.height
    }
  }

  const gridMode = Number.isSafeInteger(options.cols) && options.cols > 0 && Number.isSafeInteger(options.rows) && options.rows > 0
  const layout = gridMode
    ? tileGrid(width, height, options.cols, options.rows)
    : tileLayout(width, height, tileSize, overlap)
  ...
  for (const tile of layout.tiles) {
    ...
    let pipeline = sharp(imgSource, { failOn: 'none' }).extract({
      left: tile.x,
      top: tile.y,
      width: tile.width,
      height: tile.height,
  ...
  let overview = sharp(imgSource, { failOn: 'none' }).resize({
    width: overviewSize,
    height: overviewSize,
    fit: 'inside',
  ...
  const manifest = {
    ...
    tileSize: gridMode ? 0 : tileSize,
    overlap: gridMode ? 0 : overlap,
